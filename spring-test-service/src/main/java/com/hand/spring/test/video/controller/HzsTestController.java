...
import com.hand.spring.test.video.service.IHzsTestService;
...
public class HzsTestController {

    @Autowired
    private IHzsTestService hzsTestService;
    ...
    public ResponseEntity selectSession(@RequestParam(name = "levelNum") Long levelNum) throws TestException {
        ...
        return ResponseEntity.ok(hzsTestService.selectSession(levelNum));
    }
    ...
    public ResponseEntity select(@RequestParam(defaultValue = "1") int page,
        ...
        return ResponseEntity.ok(hzsTestService.select(page, pageSize));
    }
    ...
    public ResponseEntity selectRedis(@RequestParam String name) {
        return ResponseEntity.ok(hzsTestService.selectRedis(name));
    }
    ...
    public ResponseEntity submitSession(@RequestBody JSONObject jsonSession) {
        hzsTestService.submitSession(jsonSession);
